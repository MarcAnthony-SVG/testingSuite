...
describe('combineTwoArrays', function() {
    ...
    it("applies combines the two input arrays (1)", function() {
      expect(combineTwoArrays([1, 2, 3],[4, 5])).to.deep.eq({combinedArrays: [1, 2, 3, 4, 5]})
    })
    it("applies combines the two input arrays (2)", function() {
      expect(combineTwoArrays(['c', 4], ['pi', 5, 'd'])).to.deep.eq({combinedArrays: ['c', 4, 'pi', 5, 'd']})
    })
})