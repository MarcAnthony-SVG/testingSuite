...
describe("objTen", function () {
  it("Returns the object with respective values (1).", function () {
    expect(objTen([1, 15, 10, 10, 10, 3])).to.deep.eq({
      lessThanTen: 2,
      equalToTen: 3,
      greaterThanTen: 1,
    });
  });
  it("Returns the object with respective values (2).", function () {
    expect(objTen([23, 5, 0, 10, 90, 35, 10])).to.deep.eq({
      lessThanTen: 2,
      equalToTen: 2,
      greaterThanTen: 3,
    });
  });
  it("Returns the object with respective values (3).", function () {
    expect(objTen([67, 10, 5, 469, 10, 3, 5])).to.deep.eq({
      lessThanTen: 3,
      equalToTen: 2,
      greaterThanTen: 2,
    });
